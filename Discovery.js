...
function connectCallback() {															//connect callback function upon connecting to broker	//
   ...
   mqttClient.publish(topicToPublishTo, '- PLEASE ENTER PASSWORD BEFORE ALL COMMANDS TO EXECUTE' , publishCallback);
...
const magLockOfInterest = 'C2:90:8F:60:B7:58'
var ledChar;	
var magChar;
var checked;				
...
async function messageEventHandler(topic, message, packet) {
  	...
  	if(message.toString().toLowerCase() == "pass") {
  		checked = 1;
  		mqttClient.publish(topicToPublishTo, '- Publish "Commands" to see all the commands', publishCallback);
  		mqttClient.publish(topicToPublishTo,"PASSWORD ENTERED IS CORRECT", publishCallback);
		...
		if(checked == 1){
			...
			mqttClient.publish(topicToPublishTo,"PASSWORD NOT ENTERED", publishCallback);
		}
		checked = 0;
	}
	...
	mqttClient.publish(topicToPublishTo,"Invalid Command or incorrect password", publishCallback);
	}  
   }
